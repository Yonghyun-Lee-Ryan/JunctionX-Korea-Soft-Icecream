...
48시간이 실제로 증명한 칸은 하나입니다. 공고 한 건에서 요구사항을 얼마나 정확히 뽑아 제출 가능한 파일로 만드는가. 나머지는 아직 가정이고, 가정을 가정이라고 적어 두는 편이 낫다고 봤습니다. 그 한 칸이 서면 나머지가 그 위에서 자랍니다.

### 누가 돈을 내는가

첫 고객은 중소 SI·컨설팅 기업입니다. 연 10~20건을 검토하고 한 건당 3~5일이 드는데, 그 시간이 곧 제안 담당자 인건비입니다. 단위는 공고 한 건당 제안 착수 준비 시간입니다. 얼마나 줄이는지는 아직 숫자로 적지 않습니다. 이번에 잰 것은 시간이 아니라 정확도이고, 출처 없는 절감률을 적지 않기로 팀에서 정했습니다.

그 다음 자리가 둘 보입니다. 은행과 보험사도 IT 조달의 발주 쪽에서 같은 문서를 읽습니다. 조달 정보 서비스에 산출물 레이어로 얹는 길도 있습니다. 둘 다 확인이 더 필요한 가정입니다.

### 로드맵

| 버전 | 무엇 |
|---|---|
| v1, 지금 | 회사 하나와 응찰 한 건. 공고 해부부터 제출 준비까지 |
| v2 | 사전규격 연계, 회사 여러 곳 |
| v3 | 낙찰 사후 추적, 발주기관 카드 |
| v4 | 제안서 초안. 그때는 요구사항 조견표가 근거가 됩니다 |

### 구조가 확장을 싸게 만듭니다

지금 v1 을 이렇게 지은 이유가 다음 버전에 있습니다.

서류 갈래를 늘리는 비용이 에이전트 하나입니다. 갈래 정의와 프롬프트만 추가하면 화면과 백엔드는 그대로입니다. 직접생산확인증명서가 지금 그 자리에 갈래만 정의된 채로 있습니다.

화면이 서버 봉투를 그리기 때문에 탭이 늘어도 화면 코드가 바뀌지 않습니다. v3 의 발주기관 카드나 v4 의 제안서 초안도 탭 하나를 더 내려보내는 일이 됩니다.

회사 카드는 한 번 만들면 모든 공고에 다시 쓰입니다. 케이스 7일 캐시와 파일 해시 캐시, 끊긴 job 이어받기까지 세 겹을 둔 덕에 공고가 늘어도 문서 처리 단가가 선형으로 늘지 않습니다.

판정 프롬프트가 코드가 아니라 `agent/*.json` 파일이라, 도메인이 넓어질 때 코드를 고치지 않고 프롬프트를 늘립니다. Studio 의 Instruct 노드가 열리면 같은 파일을 그대로 Studio 로 되돌릴 수도 있습니다.

### v1 이 아직 열지 않은 자리

없는 기능을 있는 척하지 않기로 해서, 화면에서도 준비 중이라고 말합니다.

사이드바의 설정・API 키는 회사가 여러 곳이 되는 v2 에서 회사 전환과 키 관리가 들어갈 자리입니다. 공고 상세 열기와 회사 카드 행의 직접 입력은 사람이 서버 판정을 덮어쓰는 통로가 되고, 그렇게 덮어쓴 값이 다음 판정의 입력이 됩니다. 조달청 교차 확인 두 줄은 지금 미연동인데, 붙으면 서류를 올리기 전에도 자격 일부를 확인할 수 있습니다. macOS 빌드는 확인하지 못했고 iOS 타깃은 아직 만들지 않았습니다. 웹이 기본입니다.

전자입찰 투찰은 못 한 것이 아니라 하지 않기로 한 쪽입니다. 공동인증서와 보안토큰이 필요한 구간이라 사람이 직접 해야 하고, 제품의 끝을 무엇을 언제 어디에 내야 하는지로 잡은 것도 그 때문입니다.

## 레포에 없는 것

공고 원본 HWP 는 조달 기밀이라 `.gitignore` 로 막아 두었습니다. 대신 그 원본을 Studio 에 돌린 실물 출력을 `backend/fixtures/studio/` 에 커밋해 두었고, 키 없는 데모는 그것을 근거로 돕니다.

회사 서류 견본 여덟 종은 가상 회사 ㈜다온피엠씨 이름으로 만들어 `plan/Solar_for_Bid/06_데모입력/` 에 넣었습니다. 전부 데모용이고 실제 효력은 없습니다.
