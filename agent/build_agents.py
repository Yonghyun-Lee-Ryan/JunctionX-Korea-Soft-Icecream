...
WBS_PLANNER_PROMPT = (
    ...
    # 🔴 2026-08-23 실측(PG 대행 용역 · 요구사항 145건): 첫 패키지가 요구사항 50개를 한 행에 묶었다
    "- 🔴 한 패키지에 요구사항을 15개 넘게 묶지 않는다. 요구사항이 많으면(100건이 넘으면) 절·기능·서비스 "
    "단위로 쪼개 패키지 수를 늘린다 — 대략 요구사항 8~12개에 패키지 하나. 묶음이 커질수록 산출물이 흐려진다.\n\n"
    "[기간 — 지어내지 않는 자리]\n"
    "🔴 기간은 문서가 명시한 것만 적는다. 「착수 후 4주」, 「2026.09.01~09.30」처럼 인쇄된 것만이다.\n"
    # 🔴 실측: 사업기간 「2025.7.1~2028.6.30 / 3년」이 있는데도 9건 전부 「미 명시」로 왔다 — 문서가 말한 기간을 쓰라고 적는다
    "🔴 DOCUMENT_INFO의 project_period(사업기간)가 있으면, 계약기간 내내 이어지는 운영·대행·유지관리·정산·"
    "보안관제 성격의 패키지는 그 사업기간을 duration으로 쓴다 — 문서가 말한 기간이다.\n"
    "🔴 추진일정·단계별 일정(「착수 후 2개월 내 시스템 연계 완료」, 「시행 전 준비기간 3개월」)이 문서에 있으면 "
    "그 패키지의 duration으로 옮긴다. execution_context의 SCHEDULE 항목도 근거다.\n"
    # 🔴 2026-08-23 실측(AX 진단·컨설팅 RFP): 추진일정 6단계(7월~8월 … 12월)가 SCHEDULE 로 있는데 29개 전부 「미 명시」로 왔다
    "🔴 execution_context의 SCHEDULE 항목(title·content·timing)이 있으면, content가 가리키는 단계의 패키지마다 duration에 "
    "timing을 그대로 옮긴다 — 「7월~9월」이면 정확히 「7월~9월」. 단계와 패키지는 content의 낱말(DB 구축, 기능 개발, "
    "프로토타입, 안정화 …)로 맞춘다. 어느 단계에도 들지 않는 패키지만 「미 명시」다.\n"
    "🔴 그래도 문서에 기간이 없으면 duration에 정확히 「미 명시」라고 적는다. 통상적인 SI 일정으로 추정하지 "
    ...
    # 🔴 실측(AX 진단): 추진일정이 있는데 29개 전부 predecessors: [] 로 왔다 — 순서는 문서가 정한 것이다
    "🔴 문서에 추진일정이 있으면 추진일정의 순서가 곧 선후다 — 뒤 단계 패키지의 predecessors에 앞 단계의 마지막 "
    "패키지를 넣는다. 같은 단계 안에서는 산출물을 쓰는 패키지가 만드는 패키지를 선행으로 둔다. 패키지가 둘 이상인데 "
    "predecessors를 전부 빈 배열로 내지 않는다 — 착수(첫) 패키지만 빈 배열이다.\n\n"
    + NO_INVENTION + "\n\n"
    "[검산 필수]\n"
