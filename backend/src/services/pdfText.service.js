 ...
 */
/** 쪽 단위 텍스트 — 금지 표현에 쪽 번호를 붙이려면 쪽 경계가 있어야 한다 */
export async function extractPdfPages(buffer) {
  ...
  try {
    ...
    const pages = (Array.isArray(text) ? text : [String(text ?? '')]).map((t) => String(t ?? ''));
    return { pages: pages.length ? pages : [''], totalPages, chars: pages.reduce((n, t) => n + t.length, 0) };
  } catch (err) {
